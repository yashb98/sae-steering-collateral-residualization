...
def main(results="results"):
    ...
    L += ["", "Reading: crowding carries independent signal in GPT-2-small on both metrics. In Pythia-70M the crowding estimate is near zero with intervals spanning zero on both metrics, "
    ...
    if "llama_3_1_8b" in settings:
        adjusted = part[(part.setting == "llama_3_1_8b") & (part.predictor == "crowding") & (part.control == "primary")].set_index("target")
        raw_p, ct_p = adjusted.loc["collateral_raw"].p_holm, adjusted.loc["collateral_ctilde"].p_holm
        L += [f"Intervals are pointwise. Llama crowding has Holm-adjusted p = {raw_p:.3f} for raw count and {ct_p:.3f} for C-tilde. "
              + ("Neither passes the 0.05 threshold after that correction." if min(raw_p, ct_p) > 0.05 else ""), ""]
    ...
    if os.path.exists(xp):
        ...
        L += ["", "GPT-2-small's crowding statistics agree to within 0.02 across machines. Pythia's move by up to 0.12, but every Pythia "
              ...
              "absence of a detectable association, with point estimates that vary across machines. The weak frequency baselines also shift between machines.", ""]
